...
//
// The same file as CNSCP/cns-orchestrator's registry.js, plus surface() for
// Declare. Keep the two copies in step.
//
...
function createResolver(opts) {
  ...
  // For Declare: one name's versions, asked now. The request is conditional
  // (If-None-Match), so it is usually a 304. If the Registry does not answer,
  // what is held is returned, marked held: lifecycle as last held (§7.4).
  // "Not registered" is never answered from what is held.
  // Resolves { versions: Map(n -> status), held: boolean }.
  async function surface(name) {
    try {
      const s = await fetchSurface(name);
      return { versions: new Map(s.versions), held: false };
    } catch (e) {
      const s = surfaces.get(name);
      if (e.kind !== UNAVAILABLE || s === undefined) throw e;
      return { versions: new Map(s.versions), held: true };
    }
  }
  ...
  return {
    ...
    // For Declare: a name's versions now, or as last held
    surface: surface
  };
}
